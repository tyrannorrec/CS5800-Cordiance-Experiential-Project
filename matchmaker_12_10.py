...
def sortedMatch():
    ...
    for avaKey, avaVal in Avalara_dict.items():
        ...
        for targetKey, targetVal in UNSPSC_data.items():
                ...
                while avaIndex < len(avaWordList):
                    while currTargetIndex < length:
                        if avaWordList[avaIndex] == currTargetWordList[currTargetIndex]:
                            currmatchedlist.append(currTargetWordList[currTargetIndex])
                            currCount += 1
                            currTargetIndex += 1
                            break
            ...
            if currCount > maxCount and not math.isnan(currCommodityID):
                maxCount = currCount
                maxCommodityID = currCommodityID
                maxMatchedlist = currmatchedlist

        print(f"matching results: {[*set(maxMatchedlist)]}")        
